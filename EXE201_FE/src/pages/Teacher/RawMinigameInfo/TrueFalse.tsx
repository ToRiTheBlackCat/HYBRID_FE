...
interface TrueFalseProps {
    onStart: () => void;          // hàm chạy chế độ Review / Chơi
}

/* ===== helper: parse XML thành {statement, answer}[] ===== */
const extractQuestions = (xml: string) => {
    const doc = new DOMParser().parseFromString(xml, "application/xml");
    const nodes = Array.from(doc.getElementsByTagName("question"));
    return nodes.map((q) => ({
        statement: q.getElementsByTagName("statement")[0].textContent?.trim() ?? "",
        answer:
            q.getElementsByTagName("answer")[0].textContent?.toLowerCase() === "true",
    }));
};

const TrueFalse: React.FC<TrueFalseProps> = ({ onStart }) => {
    /* routing */
    ...
    const [questions, setQuestions] = useState<
        { statement: string; answer: boolean }[]
    >([]);
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (data) {
                setMinigame(data);
                setQuestions(extractQuestions(data.dataText));
            }
        ...
        <div className="min-h-screen font-sans mt-30">
            <div className="max-w-4xl mx-auto mt-6 p-4 bg-blue-200 rounded-xl shadow-md">
                ...
                <div className="mt-6">
                    ...
                    <div
                        className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 mt-4 transition-opacity duration-300 ${showQ ? "opacity-100" : "opacity-5 pointer-events-none select-none"
                            }`}
                    >
                        {questions.map((q, idx) => (
                            <div
                                key={idx}
                                className="bg-gray-100 rounded-lg p-3 shadow-sm font-medium"
                            >
                                {idx + 1}. {q.statement}
                                <span
                                    className={`ml-2 text-xs font-bold ${q.answer ? "text-blue-600" : "text-red-600"
                                        }`}
                                >
                                    ({q.answer ? "True" : "False"})
                                </span>
