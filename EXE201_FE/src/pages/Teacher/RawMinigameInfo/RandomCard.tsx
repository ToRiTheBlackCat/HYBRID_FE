...
import { Minigame, RandomCardItem } from "../../../types";
import { baseImageUrl } from "../../../config/base";
import { useParams } from "react-router-dom";

interface RandomCardProp {
    onStart: () => void
}

const RandomCard: React.FC<RandomCardProp> = ({onStart}) =>{
    ...
    useEffect(() => {
      ...
      const extractDeckFromXML = async (xmlString: string) => {
        ...
        const parsed: RandomCardItem[] = await Promise.all(
            questions.map(async (q) => {
            const text =
                q.getElementsByTagName("text")[0]?.textContent?.trim() || "";
            const imageURL =
                q.getElementsByTagName("image")[0]?.textContent?.trim() || "";

            // 🔽  chính là đoạn bạn yêu cầu "áp dụng"
